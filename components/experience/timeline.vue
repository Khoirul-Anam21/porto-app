...
import { type Experience } from '../../stores/model-store';
        ...
        <div class="py-6 px-4 md:px-8 space-y-3">
            <section class="flex gap-2 md:gap-4">
                <div class="flex justify-center  items-start  md:items-center basis-[25%] md:basis-[12%]">
                    <NuxtImg src="https://admin.anam6.my.id/assets/abe4ca22-0c0d-4a29-8cc7-7e672ec50a89"
                        class="w-full" />
                </div>
                <div class="flex-grow">
                    <p class="text-sm md:text-base">{{ props.experience.company }}</p>
                    <h4 class="text-xl font-bold md:font-normal md:text-2xl">{{ props.experience.position }}</h4>
                </div>
            </section>
            <section class="inline-flex flex-col md:flex-row gap-2 md:gap-4 md:items-center">
                <span class="py-1 px-6 bg-red-200 rounded-xl text-xs md:text-sm w-max">{{ props.experience.types }}</span>
